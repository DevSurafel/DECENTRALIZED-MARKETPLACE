...
serve(async (req) => {
  ...
  if (!meRes.ok) return new Response(JSON.stringify({ error: "Telegram unreachable" }), { status: 502, headers: corsHeaders });
  const { result } = await meRes.json();
  const botUsername = result?.username;
  if (!botUsername) return new Response(JSON.stringify({ error: "Bot has no username" }), { status: 500, headers: corsHeaders });
